...
export const findSubmitControl = (editor: HTMLElement): HTMLElement | null => {
  ...
  for (let depth = 0; ancestor && depth < 8; depth += 1, ancestor = ancestor.parentElement) scopes.push(ancestor);
  ...
  const nearby = [...document.querySelectorAll<HTMLElement>('button, [role="button"]')]
    ...
    .filter((control) => {
      ...
      return centerX >= editorRect.left + editorRect.width * 0.55
...
export const activateSubmitControl = (control: HTMLElement) => {
  ...
  if (typeof PointerEvent !== 'undefined') {
    control.dispatchEvent(new PointerEvent('pointerdown', { ...mouse, pointerId: 1, pointerType: 'mouse', isPrimary: true }));
    control.dispatchEvent(new PointerEvent('pointerup', { ...mouse, pointerId: 1, pointerType: 'mouse', isPrimary: true }));
  }
...
export const submissionAccepted = (editor: HTMLElement | null, prompt: string, control?: HTMLElement | null) => {
